...
### Simplified Progress Bar (One-line)
Easily create and manage progress bars without creating popup objects manually.

```python
def simple_progress_example(context):
    from . import ui_tools
    import time
    
    # Task A starts
    ui_tools.progress_bar_popup("task_a", 0, 100, "Task A starting...")

    # Task B starts (added to SAME popup)
    ui_tools.progress_bar_popup("task_b", 0, 100, "Task B starting...")
    time.sleep(1)

    # Updates
    ui_tools.progress_bar_popup("task_a", 50, 100, "Task A: 50%")
    time.sleep(2)
    ui_tools.progress_bar_popup("task_b", 20, 100, "Task B: 20%")
    time.sleep(1)

    # Task A finishes
    ui_tools.progress_bar_popup("task_a", 100, 100, "Task A Done!")
    time.sleep(1)

    # Task B finishes
    ui_tools.progress_bar_popup("task_b", 100, 100, "Task B Done!")
    
    # Or close programmatically
    # ui_tools.close_progress_bar_popup("my_task")
```
...
def threaded_progress_example(context):
    ...
    # 1. Create Popup with prevent_close=True, blocking=True prevents interacting with the viewport while running
    ...
    # 3. Optional: Add a Cancel Button
    # def on_cancel():
    ...
    def background_task():
        try:
            ...
            # Optional: to auto-close the popup after the task is finished
            # popup.finished = True
            
            # Optional: Change Cancel button to Close
            # cancel_btn.text = "Close"
            # cancel_btn.callback = lambda: setattr(popup, 'finished', True)
            
            # Optional: Trigger redraw to show button change
            # progress.update(100, 100, "Done!") 
